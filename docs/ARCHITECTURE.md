...
historical item lines, totals, and creation and update timestamps. A payment
summary is deferred until a Payment entity exists.
...
RestaurantTable updates. Payment is not part of this schema.
...
blocking-payment flag to be cancelled. D-016 remains the source of truth, but
transactional Payment-aware cancellation and the `Order -> Payment` protocol
cannot be integrated until the Payment model is implemented in Stage 9 or the
first payment stage.
