...
session. Stage 9 implements this Checkout boundary. Stripe webhook verification
and provider-confirmed terminal transitions remain the separate Stage 10
boundary.
...
Stage 9 intentionally has no webhook route or Stripe event table. It creates
`pending` and may persist `failed`; Stage 10 will verify webhooks and own
provider-confirmed `succeeded` and `expired` transitions.
...
planned Stripe webhook. `POST /api/v1/orders` does not create a payment. The
implemented
...
retrieval uses the same access token. The webhook route remains deferred to
Stage 10.
